...
export default function AuthForm({ mode }: Props) {
  ...
  const emailError = email.length > 0 && !isValidEmail(email) ? t.err_email : "";
  const passwordError = mode === "signup" && password.length > 0 && password.length < 8 ? t.err_pass_short : "";
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-1 mb-4">
        <input
          type="email" placeholder={t.auth_email} value={email} autoComplete="off"
          onChange={(e) => setEmail(e.target.value)} style={inputStyle(!!emailError)} className="mb-1"
        />
        {emailError && <p className="text-red-500 text-xs mb-2">{emailError}</p>}

        <div className="relative mt-2">
          <input
            type={showPassword ? "text" : "password"} placeholder={t.auth_password} value={password}
        ...
        <button type="submit" disabled={loading || !isFormValid}
          style={{ backgroundColor: "var(--text-primary)", color: "var(--bg-main)", borderRadius: "8px", marginTop: "8px", opacity: !isFormValid ? 0.4 : 1 }}
          className="w-full py-3 text-sm font-semibold transition-opacity">
          {loading ? "..." : mode === "login" ? t.auth_signin_btn : t.auth_signup_btn}
        </button>
      </form>

      <div className="flex items-center gap-3 mb-4">
        <div style={{ flex: 1, height: "1px", backgroundColor: "var(--border)" }} />
        <span style={{ color: "var(--text-secondary)" }} className="text-xs">{t.auth_or}</span>
        <div style={{ flex: 1, height: "1px", backgroundColor: "var(--border)" }} />
      </div>
